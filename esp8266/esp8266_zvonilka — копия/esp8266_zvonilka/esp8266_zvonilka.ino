...
// void sendSMSReply(String replyText) {
//   Serial.print("📤 Отправляем ответное СМС (CSMP + UCS2): ");
//   Serial.println(replyText);
...
//   // 1. Кодируем ТОЛЬКО текст сообщения в HEX UCS2
//   String hexPayload = convertToUCS2(replyText);

//   while(simSerial.available()) { simSerial.read(); }
  
//   // 2. Настраиваем параметры текста (17 - стандарт, 167 - срок жизни СМС, 0 - формат, 25 - указывать кодировку UCS2)
//   simSerial.println("AT+CSMP=17,167,0,25");
//   waitForResponse("OK", 1000);

//   // 3. Отправляем номер телефона ОБЫЧНОЙ строкой (модем сейчас в режиме GSM/ASCII и четко его примет)
//   simSerial.print("AT+CMGS=\"");
//   simSerial.print(currentAdminPhone);
//   simSerial.println("\""); 
  
//   if (waitForResponse(">")) {
//     // 4. Сюда засылаем наш русский HEX-код
//     simSerial.print(hexPayload);
//     delay(100);
//     simSerial.write(26); // Символ Ctrl+Z
