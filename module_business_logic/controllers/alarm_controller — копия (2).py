...
def handle_sms_command():
    ...
    if cmd == "STATUS":
        alarm_data = check_for_alarms().get_json()  # ← переиспользуем существующую функцию
        return jsonify({"reply": f"STATUS:OK ALARM:{alarm_data['alarm']}"})

    # STATUS:1 — статус конкретного датчика
    ...
    if len(parts) == 3 and parts[0] == "RELAY" and parts[1].isdigit() and parts[2] in ("ON", "OFF"):
        ...
        state = parts[2]
        # TODO: тут вызов вашей логики управления реле
        return jsonify({"reply": f"RELAY:{relay_id}:{state}:OK"})

    # MUTE:60 — тревога всех на N минут (заметьте: тут именно минуты, как мы и решили)
    ...
    # MUTE:1:60 — тревога конкретного датчика на N минут
