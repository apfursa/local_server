...
def process_sensor_data(last_meas, setting, mode_label, alarm_min, relay_min, relay_max, alarm_max,
                        schedules_list=None, offline_timeout=5):
    ...
    # Считаем лимит секунд на основе кастомного таймаута (переводим минуты в секунды)
    allowed_seconds = float((offline_timeout if offline_timeout else 5) * 60)

    # Датчик онлайн, если замер был менее N минут назад
    is_online = (datetime.now() - last_meas.timestamp).total_seconds() < allowed_seconds
